                ...
                <div class="table-responsive container">
                    ...
                    $start_from = ($page-1) * $num_per_page;
                         ...
                         if(isset($_POST['submit'])){
                            $search = $_POST['search'];
                             ...
                             $query = "SELECT d.*, s.Title, s.Category, s.IsPaid, s.SellingPrice FROM downloads AS d JOIN sellernotes AS s ON d.NoteID = s.ID  WHERE d.Seller = $user_id AND s.Title LIKE '{$search}%'";

                        }else{
                        
                        
                     $query = "SELECT d.*, s.Title, s.Category, s.IsPaid, s.SellingPrice FROM downloads AS d JOIN sellernotes AS s ON d.NoteID = s.ID  WHERE d.Seller = $user_id ";
                         }
                    $buyer_requests = mysqli_query($connection,$query);
                        ...
                        while($row = mysqli_fetch_assoc($buyer_requests)){
                             $count = $count + 1;
                                ...
                                $note_id = $row['NoteID'];
                                $download = "SELECT * FROM sellernotes WHERE ID= $note_id";
                                $download_query = mysqli_query($connection,$download);
                                if(!$download_query){
                                    die(mysqli_error($connection));
                                }
                            
                                $download_detail = mysqli_fetch_assoc($download_query);
                                $file_location = $download_detail['NotesPreview'];
                                 if($start_from<$i){
                    ...
                    ?>
                        <tr>
                            ...
                            <td>$<?php if($selltype == 'Paid'){ echo $row['SellingPrice'];} else{ echo '0';} ?></td>
                ...
                <nav aria-label="Page navigation example" id="pagination">
                    <ul class="pagination d-flex justify-content-center">
                        <li class="page-item  <?php if($page == 1){ echo 'disabled'; }?>">
                            <a class="page-link" href="my_sold_notes.php?page=<?php echo $page-1; ?>" aria-label="Previous">
                        ...
                        ?>
                        <li class="page-item">
                            <a class="page-link <?php if($page == $i) { echo 'active'; }?>" href="my_sold_notes.php?page=<?php echo $i ; ?>"><?php echo $i ;?></a>
                        </li>
                        <?php 
                            }
                        ?>
                        ...
                        <li class="page-item <?php if($page == $total_pages){ echo 'disabled'; }?>">
                            <a class="page-link" href="my_sold_notes.php?page=<?php echo $page-1; ?>" aria-label="Next">
                                <span aria-hidden="true">&#62;</span>
                            </a>
                        </li>
                    </ul>
