                ...
                if(isset($_POST['confirm_edit'])){
                    ...
                    if(!preg_match('/[a-zA-Z]/', $note_pages) && isset($preview_note) && isset($uploaded_note)){
                        /*query to insert in sellernotes table*/
                         ...
                         $query1 = "UPDATE sellernotes SET Title = '{$note_title}', Category = '{$note_category}', DisplayPicture = '{$display_pic}' , NoteType = '{$note_type}', NumberofPages = '{$note_pages}', Description = '{$note_description}', UniversityName = '{$institute_name}', Country = '{$note_country}', Course = '{$course_name}', CourseCode = '{$course_code}', Professor = '{$proffesor}', IsPaid = '{$sell_type1}',  SellingPrice = '$note_sell_price', NotesPreview = '{$preview_note}', ModifiedDate = now() WHERE ID = '{$note_id}'  ";
