...
;              }
                if(isset($_POST['save'])){
                    ...
                    if(!preg_match('/[a-zA-Z]/', $note_pages) && isset($preview_note) && isset($uploaded_note)){
                        /*query to insert in sellernotes table*/
                        $query1 = "INSERT INTO sellernotes(SellerID,Status,Title,Category,DisplayPicture,NoteType,NumberofPages,Description,UniversityName,Country,Course,CourseCode,Professor,IsPaid,SellingPrice,NotesPreview,CreatedDate) VALUES('{$user_id}','submitted','{$note_title}','$note_category','{$display_pic}','{ $note_type}','{$note_pages}','{$note_description}','{$institute_name}','{$note_country}','{$course_name}','{$course_code}','{$proffesor}','{$sell_type1}','$note_sell_price','{$preview_note}',now())";
